 ...
 *
 * $Id: swapoff.c,v 1.1 2000/04/11 09:23:58 drt Exp $
 *
 ...
 *
 * $Log: swapoff.c,v $
 * Revision 1.1  2000/04/11 09:23:58  drt
 * Initial revision
 *
...
static char *rcsid = "$Id: swapoff.c,v 1.1 2000/04/11 09:23:58 drt Exp $";
  ...
  if(buffer)
  ...
  else
    {
      /* we are low on memory, try to write anything on the device */
		perror("Can't get enough memory");
		exit(1);
      /*
...
int handle_disk(char *name)
{
  ...
  if((status = swapoff(name)) != 0)
    {
      perror("can't turn off swapspace");
...
int main(int argc, char ** argv)
{
  ...
  /* See if the user should really use this */
  if (getuid() > 0) {
	fprintf(stderr, "You really should use this program as root.\n");
   exit(1);
  }
